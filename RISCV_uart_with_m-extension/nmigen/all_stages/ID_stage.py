...
class ID(Elaboratable):
    ...
    def __init__(self):
        ...
        self.MUL    = 0b00000010000110011 #change add funct3 and 1 bit of funct7
    ...
    def elaborate(self,platform:Platform)->Module:
       ...
       # m.d.comb+=self.instruction.eq(0xFCE08793)#0b00000000011100110000001010110011)#0xCD40CE00)
        ...
        with m.Switch(self.it0):
            with m.Case(self.MUL,self.MULH,self.MULHSU,self.MULHU,self.DIV,self.DIVU,self.REM,self.REMU):
                m.d.comb+=self.instruction_type.eq(0b010)#Inst_type.M_type
                m.d.comb+=self.s1data_out.eq(self.s1_data_in)
                m.d.comb+=self.s2data_out.eq(self.s2_data_in)
                m.d.comb+=self.des.eq(self.instruction[7:12])
        return m
