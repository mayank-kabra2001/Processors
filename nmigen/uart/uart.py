...
class UART(Elaboratable):
    ...
    def elaborate(self, platform)->Module:
        ...
        with m.If(rx_count == 10):
            with m.If(self.tx_rdy == 0):
                with m.If(self.c8bit == 0):
                    self.ra = self.rx_data
                    self.c8bit = self.c8bit.eq(self.c8bit+Const(1))
                with m.Else():
                    self.rb = self.rx_data
                    adder = Adder(self.ra,self.rb)
                    self.res.eq(adder.out)
        with m.Else():
